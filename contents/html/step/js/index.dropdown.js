...
NAVI.dropdown=function($navi,datas){
	...
	/*엘리멘트 변수*/
	var $mNavi=$navi;
	var $mBtnPrev=$mNavi.querySelector('[data-btn="prev"]');
	var $mBtnNext=$mNavi.querySelector('[data-btn="next"]');
	...
	function addEvents(){
		/*네비 이벤트 등록*/
		$mBtnPrev.off().on('click',function(){
			if(!this.hasClass('disabled')){
				changeDepths(-1);
				makeDepth(0);
			};
		});
		$mBtnNext.off().on('click',function(){
			if(!this.hasClass('disabled')){
				changeDepths(1);
				makeDepth(0);
			};
	...
	function makeDepth(add){
		...
		for(i;i<mDepthMax;i++){
			...
			if(i>=add){
				addDepthItem($mDepths[i],datas,mDepths[i]);
			};
		};
	};
	function addDepthItem($el,datas,sIdx){
		$el.empty();
	...
	function changeDepths(adder,isPass){
		...
		switch(state){
			case 'first':
				$mBtnPrev.addClass('disabled');
				$mBtnNext.removeClass('disabled');
			break;
			case 'end':
				$mBtnPrev.removeClass('disabled');
				$mBtnNext.addClass('disabled');
			break;
			default:
				$mBtnPrev.removeClass('disabled');
				$mBtnNext.removeClass('disabled');
